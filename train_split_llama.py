...
from modeling.my_llama.configuration import AdditionalConfig, MyLlamaConfig
...
def main():
    ...
    save_path = f"./outputs/train_ckpts_llama/{experiment_name}/"
...
def train(
    *,
    ...
    if dataset_name == "mrpc":
        ...
        ds = ds.filter(lambda x: x["offensive_language"] is not None and x["tweet_hashed"] is not None)
        ds = ds.class_encode_column("offensive_language")
        ds = ds['train'].train_test_split(train_size=2000, test_size=2000, seed=123, stratify_by_column="offensive_language")
        ds = ds['train'].train_test_split(train_size=0.8, seed=123, stratify_by_column="offensive_language")
        
        ds = ds.select_columns(["tweet_hashed", "offensive_language"])
    ...
    lora_config = LoraConfig(
        ...
        # layers_pattern=["layers", "dec_layers"],
    )
