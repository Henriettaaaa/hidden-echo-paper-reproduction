...
def main():
    ...
    test_dataset = val_datasets.map(preprocess_function, batched=True).train_test_split(test_size=0.5, seed=123)["test"]
    test_dataloader = DataLoader(test_dataset, batch_size=32, shuffle=False, collate_fn=transformers.default_data_collator)

    final_auc = 0
    for _ in range(20):
        auc = evaluate_model(model, test_dataloader)
        final_auc += auc
    final_auc /= 20
