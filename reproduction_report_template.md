...
本阶段主表按照论文正文 Table 1 的组织方式呈现，即在相同隐私预算下同时报告任务性能 AUC 与 EIA 对应的 Empirical Privacy（EP）。AIA 属于属性推断攻击实验，使用 Tweet Annotation 数据集，与 Financial Phrasebank 主分类任务不同，因此作为附表单独呈现。生成任务与 Llama3 分类结果暂不纳入本阶段主报告。
...
### 9.2 HiddenEcho+ reduce1 + MI 数值问题
