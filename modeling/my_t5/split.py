...
class SplittedT5ForConditionalGeneration(T5PreTrainedModel):
        ...
        if config.lst_enable:
            if config.auto_skip:
                ...
                num_reserved_layers = sum(
                    1 for i in range(config.num_layers) if i not in config.lst_skip
                )
            reduced_hidden_size = config.d_model // config.lst_reduce_factor
            self.mi_estimators = [
                (
                    MINE(config.d_model, reduced_hidden_size, config.mi_estimator_hidden_dim).to(self.device),
                    MINE(config.d_model, reduced_hidden_size, config.mi_estimator_hidden_dim).to(self.device),
                )
    ...
    def from_t5(cls, t5: T5ForConditionalGeneration):
        ...
        encoder_state_dict = model.server_encoder.state_dict()
        for name, param in model.client_denoise.ladder_side.trans.named_parameters():
            ...
            if model.config.lst_random_init:
                continue
            if name not in encoder_state_dict:
                continue
            src = encoder_state_dict[name].data
    ...
    def set_layer_skip(
        ...
        skip_layers = set(range(self.config.num_layers)) - set(sorted_layer_indices)
        if keep_last_layer:
            skip_layers.discard(self.config.num_layers - 1)
        self.config.lst_skip = sorted(skip_layers)
        self.server_layer_select.lst_skip = self.config.lst_skip
        print(f"lst_skip: {self.config.lst_skip}")
