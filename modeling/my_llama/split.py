...
class ClientEmbeddingPart(nn.Module):
    def __init__(self, config: MyLlamaConfig, embedding: nn.Embedding, tokenizer: LlamaTokenizer=None):
        super().__init__()
        self.config = config
...
class ClientHeadPart(nn.Module):
    def __init__(self, config: MyLlamaConfig, head: nn.Linear):
        super().__init__()
        self.config = config
...
class MyLlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: MyLlamaConfig, model: LlamaModel):
...
class SplittedLlamaForSequenceClassification(LlamaPreTrainedModel):
    def __init__(
        self, config: MyLlamaConfig, ptm_model: LlamaForSequenceClassification | LlamaForCausalLM
    ):
        super().__init__(config)
        ...
        self.client_embedding = ClientEmbeddingPart(
            config, ptm_model.get_input_embeddings(), AutoTokenizer.from_pretrained(config.name_or_path)
        )

        self.server_backbone = MyLlamaModel(config, ptm_model.model)
        if config.lst_enable:
    ...
    def forward(
        ...
        if self.config.lst_enable:
            noisy_hidden_states = all_hidden_states[-1]
            ...
            hidden_states = hidden_states + noisy_hidden_states
    ...
    @classmethod
    def from_qwen2(cls, qwen2: LlamaForSequenceClassification | LlamaForCausalLM):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        cls_type = LlamaForSequenceClassification
